...
if (!file.exists("cv_states.csv"))
  download.file(
    url = url_1,
    destfile = "cv_states.csv",
    method   = "libcurl",
    timeout  = 60
    )
cv_states <- read.csv("cv_states.csv")

### FINISH THE CODE HERE ###
# load state population data
url_2 <- 'https://raw.githubusercontent.com/COVID19Tracking/associated-data/master/us_census_data/us_census_2018_population_estimates_states.csv'
if (!file.exists("state_pops.csv"))
  download.file(
    url = url_2,
    destfile = "state_pops.csv",
    method   = "libcurl",
    timeout  = 60
    )
...
# format the state and state abbreviation (abbs) variables
cv_states$state <- factor(cv_states$state)
cv_states$abbs <- factor(cv_states$abbs)
...
for (i in 1:length(state_list)) { 
  cv_subset <- subset(cv_states, state == state_list[i]) # takes subset by state 
...
### COMPLETE CODE HERE
p1 <- ggplot(cv_states, aes(x = state, y = new_cases, color = state)) +
  geom_line() + geom_point(size = .5, alpha = 0.5)
ggplotly(p1)

# Inspect outliers in new_deaths using plotly
p2 <- ggplot(cv_states, aes(x = state, y = new_deaths, color = state)) + 
  geom_line() + geom_point(size = .5, alpha = 0.5)
ggplotly(p2)
...
cv_per100 <- cv_states |> filter(date==pick.date) |> select(state, abbs, newper100k, cases, deaths) # select data
#cv_per100$state_name <- cv_per100$state
cv_per100$state <- cv_per100$abb
cv_per100$abbs <- NULL

# Create hover text
cv_per100$hover <- with(cv_per100, paste(state, '<br>', "Cases per 100k: ", newper100k, '<br>', "Cases: ", cases, '<br>', "Deaths: ", deaths))
...
# Extract the data for each state by its abbreviation
cv_per100 <- cv_states_today |>  select(state, abbs, newper100k, cases, deaths) # select data
cv_per100$state <- cv_per100$abbs
cv_per100$abbs <- NULL

# Create hover text to include state name, new cases per 100k, cases, and deaths
cv_per100$hover <- with(cv_per100, paste(state, '<br>', "new cases per 100k:", newper100k,
                                         '<br>', "cases:", cases,
                                         '<br>', "deaths:", deaths))
